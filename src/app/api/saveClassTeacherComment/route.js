...
export async function POST(req) {
    try {
        ...
        const { regNumber, comment, className, attendance, attendanceTotal } = await req.json();
        console.log("Received data:", { regNumber, comment, className, attendance });
                                                ...
                                                const studentPromises = year.students.flat().map(async (student) => {
                                                    ...
                                                    if (student.studentId.regNumber === regNumber) {
                                                        try {
                                                            ...
                                                            if (studentExam) {
                                                                console.log("hi");
                                                                studentExam.classTeachersComments = comment;
                                                                studentExam.AttendanceMark = attendance
                                                                studentExam.AttendanceTotal = attendanceTotal
                                                                console.log("hi");
                                                            }
                                                        } catch (error) {
